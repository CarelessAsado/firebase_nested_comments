    ...
    //https://www.mongodb.com/docs/manual/reference/operator/aggregation/lookup/#perform-an-uncorrelated-subquery-with--lookup
    {
      $facet: {
        commentsData: [
          ...
          //firstLevelChildren : array con FIRST LEVEL CHILDREN
          {
            $addFields: {
              firstLevelChildren: {
                $filter: {
          ...
          //LETS MAP FIRSTLEVEL CHILDREN, and for everyone,set A NEW KEY
          {
            $addFields: {
              firstLevelChildren: {
                $map: {
                  input: "$firstLevelChildren",
                  as: "item",
                  in: {
                    data: "$$item",
                    children: {
          ...
          //como el array de subcomments puede estar vacío. Al restar 1 queda en negativo, asi q lo seteamos a 0
          {
            $set: {
              totalSubcomments: {
                $cond: [
                  {
                    $lt: ["$totalSubcomments", 0],
                  },
                  0,
                  "$totalSubcomments",
                ],
